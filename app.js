    ...
    const allResults = await Promise.all(
      calendars.map(cal =>
        gapi.client.calendar.events.list({
          ...
          maxResults: 500,
          orderBy: 'startTime',
        // FIX: pass cal.id into parseGoogleEvent so we know which calendar each event belongs to
...
// FIX: accept calendarId parameter and store it on the event object
function parseGoogleEvent(e, calendarId = 'primary') {
  const member = detectMember(e.summary || '', e.description || '');
  return {
    id: e.id,
    calendarId,                          // ← stored so save/delete use the right calendar
...
async function saveEvent() {
  ...
  try {
    if (editingEventId) {
      // FIX: look up the event's actual calendarId instead of hardcoding 'primary'
