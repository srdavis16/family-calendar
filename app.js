...
async function loadEvents() {
  ...
  try {
    ...
    const allResults = await Promise.all(
      calendars.map(cal =>
        gapi.client.calendar.events.list({
          ...
          maxResults: 500,
          orderBy: 'startTime',
        }).then(resp => (resp.result.items || []).map(e => parseGoogleEvent(e, cal.id)))
          .catch(() => [])
      )
    );
    ...
    console.log('Calendar access roles:', calendars.map(c => ({ name: c.summary, id: c.id, role: c.accessRole })));
    renderCurrentView();
  } catch (err) {
...
function parseGoogleEvent(e, calendarId = 'primary') {
...
async function saveEvent() {
  ...
  if (allDay) {
    // FIX: Google requires all-day end date to be the day AFTER the start date
    const endDate = new Date(dateVal + 'T00:00:00');
    endDate.setDate(endDate.getDate() + 1);
    eventBody.start = { date: dateVal };
    eventBody.end = { date: formatDate(endDate) };
  } else {
  ...
  try {
    if (editingEventId) {
      ...
      // New events always go into the primary calendar
    ...
    if (status === 403) showToast('Permission denied — you cannot edit this event');
    else if (status === 404) showToast('Event not found — it may have been deleted');
    else if (status === 401) showToast('Session expired — please sign out and sign back in');
    else showToast('Error saving event');
  }
}
...
async function deleteEvent() {
  ...
  try {
    ...
    console.log('Deleting event:', editingEventId, 'from calendar:', calendarId);
    ...
    const status = err?.status || err?.result?.error?.code;
    if (status === 403) showToast('Permission denied — you cannot delete this event');
    else if (status === 404) showToast('Event not found — it may have already been deleted');
    else if (status === 401) showToast('Session expired — please sign out and sign back in');
    else showToast('Error deleting event');
  }
}
...
function renderMonth() {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    // FIX: guard against event clicks bubbling up to open "New Event" modal
